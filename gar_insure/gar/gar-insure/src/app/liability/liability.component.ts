...
export class LiabilityComponent implements OnInit {
  ...
  onReg(form)
  {
    if(form.valid)
    {
      this.polauthService.liability_policy(this.authService.cur_user,form.value.busit,form.value.mobile,form.value.servt,form.value.numpeople,form.value.city);
     //this.router.navigate(['/login']);
     (<HTMLInputElement>document.getElementsByClassName("busit")[0]).value=this.busit+"";
